/**
 * SIM6: FIXED with Lower Conversion Rates
 * 
 * Key Fixes from Sim5:
 * - Insights threshold: 8 → 15 check-ins
 * - Insights multiplier: 3.0x → 2.0x
 * - Added time decay for conversions
 * - Lower medium base rates (4%, 3%, 4%)
 * 
 * Expected Results:
 * - 8-12% overall conversion (vs 63% in Sim5)
 * - 40-50% get insights (vs 89% in Sim5)
 * - 2-3x insights lift (vs 7.7x in Sim5)
 */
...
const PERSONAS = [
  // HIGH ENGAGERS (10% - True Believers) - REDUCED
  {
    ...
    conversionProbability: 0.15,  // 15% → 30% with insights
    checkInRate: 0.85,
    journalRate: 0.75,
    detailsRate: 0.65,
    churnMultiplier: 0.5,
  },
  {
    ...
    engagementLevel: 'high',
    conversionProbability: 0.12,  // 12% → 24% with insights
    checkInRate: 0.80,
    journalRate: 0.70,
    detailsRate: 0.60,
    churnMultiplier: 0.6,
  ...
  // MEDIUM ENGAGERS (30% - Hopeful but Inconsistent) - REDUCED
  {
    name: 'Busy Parent',
    ...
    conversionProbability: 0.04,  // 4% → 8% with insights
    checkInRate: 0.60,
    journalRate: 0.45,
    detailsRate: 0.30,
    churnMultiplier: 1.0,
  },
  {
    ...
    engagementLevel: 'medium',
    conversionProbability: 0.03,  // 3% → 6% with insights
    checkInRate: 0.55,
    journalRate: 0.40,
    detailsRate: 0.25,
    churnMultiplier: 1.1,
  ...
  // SKEPTICS (60% - Don't Really Believe) - INCREASED!
  {
    ...
    engagementLevel: 'low',
    conversionProbability: 0.02,  // 2% → 4% with insights
    checkInRate: 0.35,
    journalRate: 0.20,
    detailsRate: 0.10,
    churnMultiplier: 1.8,
  },
  {
    ...
    engagementLevel: 'low',
    conversionProbability: 0.01,  // 1% → 2% with insights
    checkInRate: 0.30,
    journalRate: 0.15,
    detailsRate: 0.08,
    churnMultiplier: 2.0,
...
async function generateInsights(user, day) {
  // Insights available after Day 4 if enough data (RAISED from 8 to 15)
  if (day < 4 || user.hasInsights || user.checkInCount < 15) return false;
...
function attemptConversion(user, day) {
  ...
  // Insights boost (REDUCED from 3.0x to 2.0x)
  if (user.hasInsights) conversionProb *= 2.0;
  
  // Meaningful days boost
  if (user.meaningfulDays >= 3) conversionProb *= 1.3;
  
  // Time decay - more aggressive (lower chance early on)
  if (day < 5) conversionProb *= 0.2;        // Only 20% early
  else if (day < 8) conversionProb *= 0.5;   // 50% mid-way
  else conversionProb *= 1.0;                // Full probability late
...
async function simulateDay(day) {
  ...
  if (day <= 3 && simulation.users.length < CONFIG.TOTAL_USERS) {
    ...
    for (let i = 0; i < toAdd && simulation.users.length < CONFIG.TOTAL_USERS; i++) {
      const userId = `sim6_${String(simulation.users.length + 1).padStart(3, '0')}`;
...
function calculateAnalytics() {
  ...
  return {
    simulation: 'Sim6-SkepticHeavy',
    config: {
      users: CONFIG.TOTAL_USERS,
      days: CONFIG.SIM_DAYS,
      demographics: '10% High | 30% Medium | 60% SKEPTICS',
      insightsThreshold: '15 check-ins',
      insightsMultiplier: '2.0x',
    },
    users: {
...
async function runSimulation() {
  console.log('🚀 SIM6: SKEPTIC-HEAVY TEST (60% Naysayers!)');
  ...
  console.log(`👥 Demographics: 10% High | 30% Medium | 60% SKEPTICS 🤔\n`);
  ...
  fs.writeFileSync(`${CONFIG.OUTPUT_DIR}/sim6-fixed-results.json`, JSON.stringify({
    config: CONFIG,
    analytics,
  ...
  console.log('\n✅ Sim6 complete!');
  console.log(`💾 Results: ${CONFIG.OUTPUT_DIR}/sim6-fixed-results.json`);
}
