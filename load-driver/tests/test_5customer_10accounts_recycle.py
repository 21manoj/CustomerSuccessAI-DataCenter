...
"""
================================================================
New 5 Customer / 10 Accounts — Full Lifecycle Recycle Test
================================================================
DC2_S Vertical — Complete Ring 1 Platform Test (Ring 3 Harness)

Goals:
  1. Onboard 5 new customers with 10 accounts each
  2. Create UI login credentials for each customer
  3. Create demo data per journey pattern variations
  4. Run the signal analyst / analyzer
  5. Check playbook recommendations
  6. Cleanup all 5 customers, rollback DB to original state

Output: kpi-dashboard/backend/New5Customer10accounts-recycle_test.out
================================================================
"""
...
OUTPUT_FILE = os.getenv(
    ...
    os.path.join(os.path.dirname(os.path.abspath(__file__)),
                 ...
                 "New5Customer10accounts-recycle_test.out")
)

# Different pattern mixes for each customer — simulates real-world variation
PATTERN_MIXES = [
    {"crisis": 0.30, "churn": 0.20, "stable": 0.25, "expansion": 0.25},  # Balanced-risk
    {"crisis": 0.10, "churn": 0.10, "stable": 0.50, "expansion": 0.30},  # Healthy portfolio
    {"crisis": 0.40, "churn": 0.30, "stable": 0.20, "expansion": 0.10},  # High-risk portfolio
    {"crisis": 0.15, "churn": 0.15, "stable": 0.30, "expansion": 0.40},  # Growth-oriented
    {"crisis": 0.25, "churn": 0.25, "stable": 0.25, "expansion": 0.25},  # Uniform distribution
]
...
def main():
    ...
    log("=" * 70)
    log("  NEW 5 CUSTOMER / 10 ACCOUNTS — FULL LIFECYCLE RECYCLE TEST")
    ...
    # ──────────────────────────────────────────────────────────
    # Phase 1+2: Register + Onboard 5 Customers (10 accounts each)
    #
    ...
    # ──────────────────────────────────────────────────────────
    section("PHASE 1+2: Register + Onboard 5 Customers (10 accounts each)")
    ...
    for i in range(NUM_CUSTOMERS):
        ...
        log(f"      Account IDs: {acct_ids[:3]}...{acct_ids[-1] if acct_ids else ''}")
    ...
    for cust in customers:
        ...
        # Analyze first 3 accounts per customer using the full agentic loop
        sample_ids = cust["account_ids"][:3]
    ...
    # ──────────────────────────────────────────────────────────
    # Phase 6: Cleanup — delete all 5 test customers
    # ──────────────────────────────────────────────────────────
    section("PHASE 6: Cleanup — Delete All Test Customers & Rollback")

    cleanup_success = 0
    cleanup_failed = 0

    # Re-auth as superuser for cleanup
    super_client = make_client(SUPER_EMAIL, SUPER_PASSWORD)
    if not super_client:
        fail("Superuser re-login failed for cleanup")
        write_output()
        return 1

    for cust in customers:
        cid = cust["customer_id"]
        subsection(f"Cleaning up: {cust['name']} (ID={cid})")

        # Dry run first
        log(f"    Dry run...")
        dry_resp = super_client.post(f"/api/admin/cleanup/customer/{cid}", {"dry_run": True})
        if dry_resp:
            rows = dry_resp.get("rows_deleted", dry_resp.get("total_rows", "?"))
            log(f"      Would delete: {rows} rows")
        else:
            log(f"      Dry run returned None")

        # Actual delete
        log(f"    Deleting...")
        del_resp = super_client.post(f"/api/admin/cleanup/customer/{cid}", {
            "dry_run": False, "confirm": True
        })
        if del_resp:
            rows = del_resp.get("rows_deleted", del_resp.get("total_rows", "?"))
            tables = del_resp.get("tables_deleted", del_resp.get("tables_cleaned", "?"))
            ok(f"Deleted: {rows} rows across {tables} tables")
            cleanup_success += 1
        else:
            fail("Cleanup returned None")
            cleanup_failed += 1

        # Verify deletion — login should now fail
        log(f"    Verifying deletion...")
        dead_client = CSPulseClient(base_url=BASE_URL, email=cust["email"], password=cust["password"])
        if dead_client.login():
            fail("User can still login after cleanup!")
        else:
            ok("User login correctly fails after cleanup")

    # ──────────────────────────────────────────────────────────
    # Phase 7: Verify DB Rollback — baseline match
    # ──────────────────────────────────────────────────────────
    section("PHASE 7: Verify DB Rollback to Original State")

    super_client = make_client(SUPER_EMAIL, SUPER_PASSWORD)
    if super_client:
        final_accounts = super_client.get_accounts()
        final_count = len(final_accounts) if final_accounts else 0
        if final_count == baseline_accounts:
            ok(f"Account count matches baseline: {final_count} == {baseline_accounts}")
        else:
            fail(f"Account count mismatch: {final_count} != baseline {baseline_accounts}")
    else:
        fail("Superuser login failed after cleanup")
    ...
    log(f"  Cleanups: {cleanup_success} success / {cleanup_failed} failed")
    log(f"")
    ...
    log(f"\n  Customer Credentials Created (for UI login):")
    log(f"  {'Customer':<25} {'Email':<35} {'Password':<25}")
    log(f"  {'-'*25} {'-'*35} {'-'*25}")
    for cust in customers:
        log(f"  {cust['name']:<25} {cust['email']:<35} {cust['password']:<25}")
...
def write_output():
    ...
    try:
        ...
        alt_path = "/tmp/New5Customer10accounts-recycle_test.out"
        try:
