                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    {action.trigger_context && (
                      <span className="inline-flex items-center gap-1 mt-1 text-[10px] text-amber-700 bg-amber-50 px-1.5 py-0.5 rounded">
                        ...
                        {(action.trigger_context.signal_type || '').replace(/_/g, ' ')}
                      </span>
                    )}
                  </div>
    ...
    return (
      <div className="flex-1 flex flex-col min-h-0">
        ...
        <div className="flex-shrink-0 px-6 pt-5 pb-3 border-b border-gray-200 bg-white">
          ...
          {/* "Why this action?" — trigger context */}
          {action.trigger_context && (
            <div className="mt-2 flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
              <svg className="w-3.5 h-3.5 text-amber-600 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold text-amber-800">Why this action?</p>
                <p className="text-xs text-amber-700 mt-0.5">{action.trigger_context.signal_summary}</p>
                <div className="flex items-center gap-2 mt-1 text-[10px] text-amber-600">
                  <span className="bg-amber-100 px-1.5 py-0.5 rounded font-medium">{(action.trigger_context.signal_type || '').replace(/_/g, ' ')}</span>
                  {action.trigger_context.signal_date && <span>{action.trigger_context.signal_date}</span>}
                  {action.trigger_context.sentiment && action.trigger_context.sentiment !== 'neutral' && (
                    <span className={action.trigger_context.sentiment === 'positive' || action.trigger_context.sentiment === 'very_positive' ? 'text-green-600' : 'text-red-600'}>
                      {action.trigger_context.sentiment.replace(/_/g, ' ')}
                    </span>
                  )}
                </div>
              </div>
