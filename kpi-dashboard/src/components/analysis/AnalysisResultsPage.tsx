...
import { classifyColor, classifyBgClass } from '../../utils/healthThresholds';
import {
...
const AnalysisResultsPage: React.FC = () => {
  ...
  const getHealthColor = (score: number) => {
    const hex = classifyColor(score);
    return hex === '#16a34a' ? 'text-green-600' : hex === '#ca8a04' ? 'text-yellow-600' : 'text-red-600';
  };

  const getHealthBgColor = (score: number) => classifyBgClass(score);
