...
class CustomerPlaybook(db.Model):
    """Per-customer playbook definitions (overrides or extends system playbooks).

    System playbooks (is_system=True) are seeded from PLAYBOOK_CONFIG on first access.
    Custom playbooks (is_system=False) are created by the customer via the library API
    or generated by Claude via the generate_playbook_from_description MCP tool.
    """
    __tablename__ = 'customer_playbooks'

    id = db.Column(db.Integer, primary_key=True)
    customer_id = db.Column(db.Integer, db.ForeignKey('customers.customer_id'), nullable=False, index=True)
    playbook_id = db.Column(db.String(50), nullable=False)  # 'PB-CUSTOM-01' for custom, 'PB-01' for override
    name = db.Column(db.String(200), nullable=False)
    description = db.Column(db.Text)
    is_system = db.Column(db.Boolean, default=False)
    trigger_conditions = db.Column(db.JSON)   # [{"kpi_code": "P3-KPI3", "operator": "less_than", "threshold": 30}]
    trigger_logic = db.Column(db.String(10), default='OR')  # AND / OR
    actions = db.Column(db.JSON)              # [{"step": 1, "description": "...", "owner_role": "csm", "estimated_hours": 4}]
    auto_trigger_enabled = db.Column(db.Boolean, default=False)
    cooldown_hours = db.Column(db.Integer, default=168)  # 7 days
    created_by = db.Column(db.String(100))    # 'system' or user email
    created_at = db.Column(db.DateTime, server_default=db.func.now())
    updated_at = db.Column(db.DateTime, server_default=db.func.now(), onupdate=db.func.now())
