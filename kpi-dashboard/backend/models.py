...
class CustomerPlaybook(db.Model):
    """Per-customer playbook templates with RACI, multi-backend execution, and approval flow.

    CS Pulse is the system of record — actual execution happens via Claude Skills,
    n8n workflows, manual CSM work, or API calls. Each step carries RACI roles.
    Status flow: draft → (VP CS approves) → active → archived.
    System playbooks (is_system=True) are seeded from PLAYBOOK_CONFIG on first access
    and cannot be deleted. Custom playbooks (is_system=False) are created by the customer
    via the library API or generated by Claude via the generate_playbook_from_description MCP tool.
    """
    __tablename__ = 'customer_playbooks'

    id = db.Column(db.Integer, primary_key=True)
    customer_id = db.Column(db.Integer, db.ForeignKey('customers.customer_id'), nullable=False, index=True)
    vertical = db.Column(db.String(50), nullable=True)  # 'dc2_s', 'saas_premium', None=cross-vertical
    ...
    trigger_conditions = db.Column(db.JSON)   # [{kpi_code, operator, threshold}]
    trigger_kpis = db.Column(db.JSON)         # ['P5-KPI1', 'P3-KPI3']
    trigger_logic = db.Column(db.String(10), default='OR')

    # Steps with RACI + execution backend
    # Each step: {id, name, description, estimated_hours,
    #   responsible, accountable, consulted: [], informed: [],
    #   execution_backend: manual|claude_skill|n8n_webhook|api_call,
    #   backend_config: {skill_name, webhook_url, ...}}
    steps = db.Column(db.JSON)
    actions = db.Column(db.JSON)  # Legacy format for backward compat

    estimated_hours = db.Column(db.Float)
    estimated_duration_days = db.Column(db.Integer)
    automation_level = db.Column(db.String(20), default='partial')
    auto_trigger_enabled = db.Column(db.Boolean, default=False)
    cooldown_hours = db.Column(db.Integer, default=168)

    # Governance (draft → approve → active)
    ...
    __table_args__ = (
        db.UniqueConstraint('customer_id', 'playbook_id', name='uq_customer_playbook'),
        db.Index('idx_cp_customer', 'customer_id'),
        db.Index('idx_cp_status', 'customer_id', 'status'),
    )
    ...
    def to_dict(self):
        return {
            ...
            'trigger_kpis': self.trigger_kpis,
...
class PlaybookStepLog(db.Model):
    """Step-level execution log for RAG learning.

    Each row records what happened during one step of a playbook execution.
    The input_context + output_result + decision_rationale fields form the
    RAG corpus for improving future playbook recommendations.
    """
    ...
    execution_backend = db.Column(db.String(30))
    ...
    __table_args__ = (
        db.Index('idx_psl_execution', 'execution_id', 'step_id'),
        db.Index('idx_psl_customer', 'customer_id'),
    )

    def to_dict(self):
        return {
            'id': self.id, 'execution_id': self.execution_id,
            'step_id': self.step_id, 'step_name': self.step_name,
            'status': self.status, 'executed_by': self.executed_by,
