...
3. Explicit customer_id
4. Custom weights
5. Configurable num_accounts
6. User creation
7. Directory provisioning
8. CSV file generation
9. Idempotency
10. Error handling
"""
...
API_ENDPOINT = f"{BASE_URL}/api/onboarding/complete"
...
def test_directory_provisioning(customer_id: int):
    ...
    checks = [
        ...
        ((customer_dir / "journey").exists(), "journey/ subdirectory exists"),
    ]
...
def test_idempotency():
    ...
    try:
        ...
        if response1.status_code == 200 and response2.status_code in [200, 400]:
            # Second request should either succeed (update) or fail gracefully (already exists)
            data2 = response2.json()
            if response2.status_code == 200:
                print_result(True, "Second request succeeded (updated existing)")
            elif "already exists" in str(data2.get('error', '')):
            ...
            print_result(False, f"Requests failed: {response1.status_code}, {response2.status_code}")
            return False
