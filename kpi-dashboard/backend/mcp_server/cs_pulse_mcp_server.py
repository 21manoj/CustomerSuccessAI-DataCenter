...
def export_customer_csvs(
    ...
    Exported files (matching config/csv_schemas.json):
      Regular model: accounts.csv, kpi_measurements.csv, enhanced_qualitative_signals.csv, products.csv
      Context graph: stakeholders.csv, engagement_events.csv, account_business_profiles.csv,
                     outcomes.csv, decisions.csv, signal_edges.csv, industry_benchmarks.csv
    ...
    with app.app_context():
        ...
        from models import (
            ...
            QualitativeSignal, ContextNode, ContextEdge,
        )
        from extensions import db
        ...
        if account_ids:
            ...
            if node_ids:
                ctx_edges_raw = ContextEdge.query.filter(
        ...
        # Build node_id -> source_ref lookup for edge CSV
        node_ref_map = {}
        for n in ctx_nodes:
            ref = n.source_ref or n.source_event_id or f'node_{n.node_id}'
            node_ref_map[n.node_id] = ref
        ...
        # ----------------------------------------------------------
        # 9. decisions.csv (node_type=DECISION)
        # ----------------------------------------------------------
        dec_cols = [
            'account_id', 'decision_date', 'title', 'decision_maker_role',
            'chosen_option', 'decision_id', 'options_considered', 'revenue_impact',
            'risk_level', 'evidence_refs', 'outcome_description', 'confidence',
        ]
        dec_rows = []
        for n in nodes_by_type.get('DECISION', []):
            props = n.properties or {}
            dec_rows.append({
                'account_id': n.account_id,
                'decision_date': n.occurred_at.isoformat() if n.occurred_at else '',
                'title': n.title or '',
                'decision_maker_role': props.get('decision_maker_role', n.node_subtype or ''),
                'chosen_option': props.get('chosen_option', ''),
                'decision_id': n.source_ref or n.source_event_id or '',
                'options_considered': props.get('options_considered', ''),
                'revenue_impact': float(n.revenue_impact) if n.revenue_impact else '',
                'risk_level': props.get('risk_level', ''),
                'evidence_refs': props.get('evidence_refs', ''),
                'outcome_description': props.get('outcome_description', ''),
                'confidence': float(n.confidence) if n.confidence else '',
            })
        results['decisions.csv'] = _write_csv('decisions.csv', dec_cols, dec_rows)

        # ----------------------------------------------------------
        # 10. signal_edges.csv (context_edges table)
        # ----------------------------------------------------------
        se_cols = [
            'from_signal_ref', 'to_signal_ref', 'edge_type', 'weight',
            'confidence', 'lag_days', 'revenue_impact', 'evidence',
            'source_platform', 'created_by', 'evidence_type',
            'evidence_description', 'kpi_code', 'kpi_value', 'decision_ref',
        ]
        se_rows = []
        for e in ctx_edges_raw:
            props = e.properties or {}
            se_rows.append({
                'from_signal_ref': node_ref_map.get(e.from_node_id, f'node_{e.from_node_id}'),
                'to_signal_ref': node_ref_map.get(e.to_node_id, f'node_{e.to_node_id}'),
                'edge_type': e.edge_type or '',
                'weight': float(e.weight) if e.weight else '',
                'confidence': float(e.confidence) if e.confidence else '',
                'lag_days': e.lag_days if e.lag_days is not None else '',
                'revenue_impact': float(e.revenue_impact) if e.revenue_impact else '',
                'evidence': props.get('evidence', ''),
                'source_platform': e.source_platform or '',
                'created_by': e.created_by or '',
                'evidence_type': props.get('evidence_type', ''),
                'evidence_description': props.get('evidence_description', ''),
                'kpi_code': props.get('kpi_code', ''),
                'kpi_value': props.get('kpi_value', ''),
                'decision_ref': props.get('decision_ref', ''),
            })
        results['signal_edges.csv'] = _write_csv('signal_edges.csv', se_cols, se_rows)

        # ----------------------------------------------------------
        # 11. industry_benchmarks.csv (node_type=EXTERNAL_CONTEXT)
        # ----------------------------------------------------------
        ib_cols = [
        ...
        ib_rows = []
        for n in nodes_by_type.get('EXTERNAL_CONTEXT', []):
            props = n.properties or {}
            ib_rows.append({
                'kpi_code': props.get('kpi_code', ''),
                'benchmark_source': props.get('benchmark_source', n.source_platform or ''),
                'industry_p50': props.get('industry_p50', ''),
                'industry_p25': props.get('industry_p25', ''),
                'industry_p75': props.get('industry_p75', ''),
                'account_percentile': props.get('account_percentile', ''),
                'sample_size': props.get('sample_size', ''),
                'benchmark_date': n.occurred_at.isoformat() if n.occurred_at else '',
            })
        results['industry_benchmarks.csv'] = _write_csv(
            'industry_benchmarks.csv', ib_cols, ib_rows
        )
