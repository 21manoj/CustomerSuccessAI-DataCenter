#!/usr/bin/env python3
"""
Generate Synthetic Customer Data - Enhanced Version
Supports journey patterns and DEMO_MANIFEST generation
"""
import sys
import argparse
...
parser = argparse.ArgumentParser(description='Generate synthetic customer data with journey patterns')
...
parser.add_argument('--num-accounts', type=int, help='Number of accounts to generate (if accounts not provided)')
parser.add_argument('--journey-patterns', type=str, help='Journey pattern type (e.g., DEMO_MANIFEST)')
...
# Handle journey patterns
journey_patterns = args.journey_patterns
if journey_patterns:
    print(f"Using journey patterns: {journey_patterns}")
...
with app.app_context():
    ...
    # Generate data
    accounts = []
    num_accounts = args.num_accounts or 3
    ...
    if args.accounts:
        ...
        # Generate accounts based on customer_id
    ...
    # Generate DEMO_MANIFEST.md if journey-patterns is specified
    if journey_patterns and journey_patterns.upper() == 'DEMO_MANIFEST':
        manifest_path = customer_dir / 'DEMO_MANIFEST.md'
        manifest_content = f"""# Demo Manifest for Customer {customer_id}

Generated: {datetime.now().isoformat()}

## Journey Patterns

This customer data was generated with demo manifest journey patterns.

## Accounts

{len(accounts)} accounts generated:
"""
        for account in accounts:
            manifest_content += f"- Account {account['account_id']}: {account['account_name']}\n"
        
        manifest_content += f"""
## Data Summary

- KPIs: {len(all_kpis)} total KPIs
- KPI Measurements: {len(kpis)} records
- Time Range: {args.months} months
- Accounts: {len(accounts)}
- Signals: {len(signals)}

## Pattern Mix

Default pattern distribution:
- Crisis: 20%
- Churn: 15%
- Stable: 40%
- Expansion: 25%
"""
        ...
        print(f"✅ Generated DEMO_MANIFEST.md: {manifest_path}")
