...
for account in accounts_data:
    params = {
        ...
        'metadata': json.dumps(account['medata'])
    }
    
    # ✅ FIXED: Use :param style, not %(param)s
    conn.execute(text("""
...
# ============================================================
# LOAD KPI DATA (Sample - 12 months for each account)
# ============================================================
...
for account in accounts_data:
  for month in range(12):
        ...
        # Sample KPIs for this account/month
        kpis = [
            {
                'account_id': account['account_id'],
                'kpi_code': 'P3-KPI1',
                'value': 78 - (month * 2),  # GPU utilization declining
                'target': 65,
            ...
            },
            {
                'account_id': account['account_id'],
                'kpi_code': 'P2-KPI1',
                'value': 1.8 + (month * 0.15),  # RMA rate increasing
                'target': 2.6,
                'month': month_date.strftime('%Y-%m')
            }
        ]
...
signals = [
    ...
    },
    {
     ...
     from_contact': 'David Park (CTO)',
        ...
        'summary': 'CTO mentioned GPU utilization at 94%, workload growing 15%/month, need more capacity.',
        'sentiment': 'positive',
        'priority': 'high',
...
print("=" * 60)
print(f"\nLoaded:")
print(f"  en(accounts_data)} DC2_S accounts")
