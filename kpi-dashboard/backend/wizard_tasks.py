...
#    wizard_run.mark_started()  # Skipped - no database
#    self.update_progress  # Skipped - no database(run_id, 'phase1_generate', 5, 'Starting data generation')
    ...
    try:
        ...
        if wizard_run: wizard_run.mark_failed(error_msg)
        raise
...
def test_predictions_task(self, previous_result, run_id, version):
    ...
    if WizardRun is not None:
        try:
            ...
            pass
#    self.update_progress  # Skipped - no database(run_id, phase_name, 10, f'Starting {version.upper()} testing')
...
def learn_ensemble_weights_task(self, previous_result, run_id):
    ...
    if WizardRun is not None:
        try:
            ...
            pass
#    self.update_progress  # Skipped - no database(run_id, phase_name, 10, 'Starting ensemble learning')
...
def generate_visualizations_task(self, previous_result, run_id):
    ...
    if WizardRun is not None:
        try:
            ...
            pass
#    self.update_progress  # Skipped - no database(run_id, phase_name, 10, 'Starting visualization generation')
    ...
    try:
        ...
        if wizard_run: wizard_run.mark_completed(viz_results)
