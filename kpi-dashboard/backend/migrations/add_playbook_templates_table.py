#!/usr/bin/env python3
"""
Migration: Create customer_playbooks and playbook_step_logs tables
==================================================================
...
Tables created:
  - customer_playbooks : Per-customer playbook definitions with steps, RACI, triggers
...
Usage:
    python -m migrations.add_customer_playbooks_table

Rollback:
    DROP TABLE IF EXISTS playbook_step_logs;
    DROP TABLE IF EXISTS customer_playbooks;
"""
...
def run_migration():
    """Create customer_playbooks and playbook_step_logs tables, seed system playbooks."""
    ...
    with app.app_context():
        ...
        # ── Create customer_playbooks table ──
        ...
        """))
        conn.execute(db.text("""
            CREATE INDEX IF NOT EXISTS idx_pt_customer_status ON customer_playbooks(customer_id, status)
        """))
        conn.execute(db.text("""
            CREATE INDEX IF NOT EXISTS idx_pt_vertical ON customer_playbooks(vertical)
        """))
        print("  Created customer_playbooks table")
        ...
        for customer in customers:
            ...
            for pb in SYSTEM_PLAYBOOKS:
                try:
                    conn.execute(db.text("""
                        INSERT INTO customer_playbooks
...
if __name__ == '__main__':
    run_migration()
    print("Migration complete: customer_playbooks + playbook_step_logs")
