#!/usr/bin/env python3
"""
Delete Customers 94-108 and Verify Deletion
===========================================

This script deletes customers with IDs between 94 and 108 (inclusive),
including all related data, customer directories, and customer-specific API routes.

Usage:
    python3 delete_customers_94_108.py [--dry-run] [--verify-only]
...
def get_customer_summary(customer_ids):
    ...
    with app.app_context():
        ...
        for customer_id in customer_ids:
            ...
            kpi_measurement_count = 0
            if account_ids:
                result = db.session.execute(text('''
                    SELECT COUNT(*) FROM kpi_measurements 
                    WHERE account_id = ANY(:account_ids)
                '''), {'account_ids': account_ids})
                kpi_measurement_count = result.scalar()
            
            # Count qualitative signals
            qualitative_signal_count = 0
            if account_ids:
                result = db.session.execute(text('''
                    SELECT COUNT(*) FROM qualitative_signals 
                    WHERE account_id = ANY(:account_ids)
                '''), {'account_ids': account_ids})
                qualitative_signal_count = result.scalar()
            
            # Count account products
            account_product_count = 0
            if account_ids:
                result = db.session.execute(text('''
                    SELECT COUNT(*) FROM account_products 
                    WHERE account_id = ANY(:account_ids)
                '''), {'account_ids': account_ids})
                account_product_count = result.scalar()
            ...
            summary[customer_id] = {
                ...
                'kpi_measurements': kpi_measurement_count,
                'qualitative_signals': qualitative_signal_count,
                'account_products': account_product_count,
...
def delete_customer_data(customer_id, dry_run=False):
    ...
    try:
        ...
        # 10. Delete KPI reference ranges
        ...
        # 16. Delete child records of accounts (in correct order)
        if account_ids:
            # 16.1. Delete account_products (FK: account_id, product_id)
            deleted_counts['account_products'] = db.session.execute(
                text("DELETE FROM account_products WHERE account_id = ANY(:account_ids)"),
                {"account_ids": account_ids}
            ).rowcount
            
            # 16.2. Delete KPI measurements (FK: account_id, kpi_code)
            deleted_counts['kpi_measurements'] = db.session.execute(
                text("DELETE FROM kpi_measurements WHERE account_id = ANY(:account_ids)"),
                {"account_ids": account_ids}
            ).rowcount
            
            # 16.3. Delete qualitative signals (FK: account_id)
            deleted_counts['qualitative_signals'] = db.session.execute(
                text("DELETE FROM qualitative_signals WHERE account_id = ANY(:account_ids)"),
                {"account_ids": account_ids}
            ).rowcount
            
            # 16.4. Delete account_profiles (FK: account_id)
            deleted_counts['account_profiles'] = db.session.execute(
                text("DELETE FROM account_profiles WHERE account_id = ANY(:account_ids)"),
                {"account_ids": account_ids}
            ).rowcount
        
        # 17. Delete products (FK: customer_id, account_id)
...
def check_customer_journey_api(customer_id):
    """Check if customer has a journey API file that needs to be removed"""
    backend_dir = Path(__file__).parent
    verticals_dir = backend_dir / "verticals"
    vertical_slugs = ['DC2_S', 'dc2_s', 'DC2S', 'dc2s']
    ...
    journey_api_files = []
    for vertical_slug in vertical_slugs:
        journey_api_path = verticals_dir / f"customer{customer_id}-{vertical_slug}" / "journey" / f"customer{customer_id}_journey_api.py"
        if journey_api_path.exists():
            journey_api_files.append(str(journey_api_path))
    
    return journey_api_files
...
def verify_deletion(customer_ids):
    ...
    with app.app_context():
        ...
        for customer_id in customer_ids:
            ...
            # Check journey API file
            journey_api_exists = len(check_customer_journey_api(customer_id)) > 0
            ...
            results[customer_id] = {
                ...
                'journey_api_exists': journey_api_exists,
                'fully_deleted': customer is None and user_count == 0 and account_count == 0 and not dir_exists and not journey_api_exists
            }
...
def main():
    parser = argparse.ArgumentParser(description='Delete customers 94-108 and verify deletion')
    ...
    customer_ids = list(range(94, 109))  # 94 to 108 inclusive
    
    print("="*70)
    print("DELETE CUSTOMERS 94-108")
    print("="*70)
    print(f"Customer IDs: {customer_ids[0]} to {customer_ids[-1]} (total: {len(customer_ids)})")
    ...
    with app.app_context():
        ...
        for customer_id, data in summary.items():
            if data is None:
                # Don't print for non-existent customers to reduce noise
                pass
            else:
                ...
                print(f"    - KPI Measurements: {data['kpi_measurements']}")
                print(f"    - Qualitative Signals: {data['qualitative_signals']}")
                print(f"    - Account Products: {data['account_products']}")
                ...
                # Check for journey API
                journey_apis = check_customer_journey_api(customer_id)
                if journey_apis:
                    print(f"    - Journey API files: {len(journey_apis)}")
                    for api_file in journey_apis:
                        print(f"      * {api_file}")
                print()
        
        if not existing_customers:
            print("✅ No customers found in range 94-108")
            return 0
        
        print(f"Found {len(existing_customers)} customers to delete")
        ...
        print("   - All database records (customers, users, accounts, KPIs, etc.)")
        print("   - All customer directories")
        print("   - All customer-specific journey API files")
        ...
        for customer_id in existing_customers:
            ...
            try:
                ...
                # Delete directory (this will also remove journey API files)
                ...
                total_records = sum(v for k, v in counts.items() if k != 'customer_id')
                print(f"    - Database records deleted: {total_records}")
                if deleted_dirs:
        ...
        for customer_id, result in verification.items():
            if result['fully_deleted']:
                deleted_count += 1
                # Only print if customer existed before
                ...
                if result['journey_api_exists']:
                    issues.append("journey API file")
                if issues:
        ...
        if all_deleted:
            print("✅ ALL CUSTOMERS 94-108 SUCCESSFULLY DELETED AND VERIFIED")
        else:
