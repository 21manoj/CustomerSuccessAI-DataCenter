...
COLLECTION_NAME = f'kpi_dashboard_vectors_customer_17'
EMBEDDING_MODEL = 'sentence-transformers/all-MiniLM-L6-v2'
BATCH_SIZE = 50  # Upload 50 points at a time

print("=" * 80)
print("CUSTOMER 17 EMBEDDINGS GENERATOR - Batched Upload")
print("=" * 80)
print()
...
if signals:
    ...
    for i, signal in enumerate(signals):
        # Generate integer ID from signal_id hash (Qdrant accepts integers or UUIDs)
        signal_id_str = str(signal['signal_id'])
        signal_id_hash = int(hashlib.md5(signal_id_str.encode()).hexdigest()[:15], 16)
        point = PointStruct(
            id=signal_id_hash,
...
if kpis:
    ...
    for i, kpi in enumerate(kpis):
        # Generate integer ID from kpi_code hash (Qdrant accepts integers or UUIDs)
        kpi_id_hash = int(hashlib.md5(kpi['kpi_code'].encode()).hexdigest()[:15], 16)
        point = PointStruct(
            id=kpi_id_hash,
            vector=embeddings[i].tolist(),
            payload={
