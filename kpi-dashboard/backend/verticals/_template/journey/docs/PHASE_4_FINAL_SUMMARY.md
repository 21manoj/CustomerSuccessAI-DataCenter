...
### **Account {ACCOUNT_ID_START} (Proactive-Growth) - 1 Milestone**
```
...
### **Account {ACCOUNT_ID_START} - Prediction at Week 5**
```json
{
  "predicted_outcome": "expansion",
  "expansion_probability": 85,
...
# Creates collection: journey_training_vectors_customer_{CUSTOMER_ID}
# Loads: 187 events + 3,220 KPIs with embeddings
# Isolated from: kpi_dashboard_vectors_customer_{CUSTOMER_ID} (production)
```

**Why Separate Collection:**
- ✅ Zero interference with production data
- ✅ Account {ACCOUNT_ID_START} exists in BOTH production and training
...
Contains: Real production accounts {ACCOUNT_ID_START}-10010
...
```
Collection: journey_training_vectors_customer_{CUSTOMER_ID}
Contains: Synthetic accounts {ACCOUNT_ID_START}, {ACCOUNT_ID_START+2}, 10007
...
results = client.search(
    collection_name="journey_training_vectors_customer_{CUSTOMER_ID}",
...
prod_results = client.search(
    ...
    query_filter={"must": [{"key": "account_id", "match": {"value": {ACCOUNT_ID_START}}}]}
)
