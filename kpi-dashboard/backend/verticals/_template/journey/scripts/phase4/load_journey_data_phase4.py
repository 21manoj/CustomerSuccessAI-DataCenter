...
def generate_sample_predictions(session, journey_account_id, account_id, final_outcome, milestones_file):
    ...
    elif account_id == '{ACCOUNT_ID_START}':  # Proactive-Growth
...
def load_metadata(session, phase, accounts_loaded, events_loaded, kpis_loaded):
    ...
    patterns_list = [
        ...
        {'account_id': '{ACCOUNT_ID_START}', 'pattern': 'proactive_growth', 'outcome': 'expanded'}
    ]
...
def main():
    ...
    try:
        # Load each account
        accounts = ['10007', '{ACCOUNT_ID_START+2}', '{ACCOUNT_ID_START}']
        total_events = 0
        total_kpis = 0
