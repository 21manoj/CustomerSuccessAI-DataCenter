...
**Environment:** Customer ID = 9, Account IDs {ACCOUNT_ID_START}-10010
...
Account IDs: {ACCOUNT_ID_START}-10010
...
Narrative Accounts: {ACCOUNT_ID_START}, {ACCOUNT_ID_START+2}, 10007
```
...
By Account:
├─ {ACCOUNT_ID_START}-{ACCOUNT_ID_START+1}, 10004-10010: 396 measurements each (12 months × 33 KPIs)
...
By Account:
├─ {ACCOUNT_ID_START} Success Story: 10 executions
...
Range: 35 (Account 10007 Feb crisis) - 99 (Account {ACCOUNT_ID_START} Dec)
```
...
├─ {ACCOUNT_ID_START}: 7 high-readiness months
...
### **✅ Account {ACCOUNT_ID_START}: Success Story (CloudScale AI Labs)**
```
...
├─ {ACCOUNT_ID_START}: 10 executions → 100% ARR growth
...
├─ {ACCOUNT_ID_START}: Detected at 78% util, 18% growth → $5M expansion
...
**Test Environment:** Isolated (customer_id = {CUSTOMER_ID})
