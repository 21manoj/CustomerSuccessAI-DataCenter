#!/usr/bin/env python3
"""
Validate Customer {CUSTOMER_ID} Data & Qdrant Integration
==============================================
...
Updated: Jan 6, 2026 (Customer {CUSTOMER_ID} Production)
...
print("=" * 80)
print("CUSTOMER 9 DATA & QDRANT VALIDATION")
print("=" * 80)
...
with engine.connect() as conn:
    ...
    for table, exp in expected.items():
        try:
            if table in ["accounts", "qualitative_signals", "kpi_measurements", "account_profiles",
                        ...
                        "account_products"]:
                # Check Customer {CUSTOMER_ID} specific data
                if table == "accounts":
                    query = f"SELECT COUNT(*) FROM {table} WHERE customer_id = {CUSTOMER_ID}"
                elif table in ["kpi_definitions", "products", "partner_definitions"]:
                    query = f"SELECT COUNT(*) FROM {table}"
                else:
                    query = f"SELECT COUNT(*) FROM {table} WHERE account_id IN (SELECT account_id FROM accounts WHERE customer_id = {CUSTOMER_ID})"
            else:
                query = f"SELECT COUNT(*) FROM {table}"
    ...
    quality_checks = [
        {
            ...
            "query": """
                ...
                WHERE ap.account_id IS NULL AND a.customer_id = {CUSTOMER_ID}
            """,
            "expected": 0
        },
        {
    ...
    result = conn.execute(text("""
        SELECT COUNT(*) FROM accounts WHERE customer_id = {CUSTOMER_ID}
    """))
    ...
    result = conn.execute(text("""
        ...
        WHERE ap.account_id IN (SELECT account_id FROM accounts WHERE customer_id = {CUSTOMER_ID})
    """))
...
test_accounts = [{ACCOUNT_ID_START}, {ACCOUNT_ID_START+2}, 10007]  # Success, churned, recovery
...
if all_ready and len(table_issues) == 0 and len(quality_issues) == 0:
    ...
    print("\n📋 Next Steps:")
    print("  1. Test Signal Analyst with Customer {CUSTOMER_ID} accounts")
    ...
    print("\n📋 Recommended Actions:")
    if table_issues:
        print("  1. Re-run data loader: python3 scripts/02_load_customer{CUSTOMER_ID}_data.py")
    if not all_ready:
