# Customer 5 Test Environment - Complete Setup Guide
...
Complete SQL and Python scripts for setting up Customer 5 test environment with Signal Analyst integration, incorporating the latest Qdrant architecture changes from January 4-5, 2026.
...
Complete PostgreSQL schema for Customer 5 test environment.
...
### 2. **02_load_customer5_data.py** (14 KB)
Loads all Customer 5 CSV files into PostgreSQL.
...
- Customer 5 specific validation
...
# Run loader
python3 02_load_customer5_data.py
...
- Collection: `kpi_dashboard_vectors_customer_5`
...
#   🎉 Customer 5 test environment is ready!
```
...
**Step 2: Load Customer 5 Data (3 min)**
```bash
python3 02_load_customer5_data.py
...
## 🎯 Customer 5 Test Data Summary

### Accounts (10 Total)

| Account ID | Type | Journey | Health | ARR | Status |
|------------|------|---------|--------|-----|--------|
| 15000 | Success | Healthy Expansion | 99 | $10.0M | ✅ Reference |
| {ACCOUNT_ID_START+1} | Recovery | Recovery Phase | 87 | $4.2M | ✅ Recovering |
| {ACCOUNT_ID_START+2} | Failure | Churned | 25 | $0 | ❌ Churned |
| 15004 | Growth | Rapid Growth | 98 | $9.6M | ✅ Strategic |
| 15005 | Stable | Steady State | 85 | $4.8M | ✅ Healthy |
| 15006 | At Risk | Declining | 55 | $1.3M | ⚠️ At Risk |
| 15007 | Recovery | Crisis Recovery | 93 | $3.0M | ✅ Recovered |
| 15008 | Strategic | Strategic Expansion | 95 | $8.2M | ✅ Strategic |
| 15009 | At Risk | Budget Constrained | 55 | $850K | ⚠️ At Risk |
| 15010 | Transition | Champion Transition | 72 | $2.4M | ⚠️ Transition |
...
results = qdrant.search(
    collection_name="kpi_dashboard_vectors_customer_5",
    query_vector=embedding,
    limit=20
)
...
account_signals = [
    r for r in results 
    if r.payload.get('account_id') == 15006
]
...
**3. Test with Customer 5:**
```bash
# In UI or API
GET /api/signal_analyst/analyze?account_id=15000&customer_id = 5
...
# Should see: 'kpi_dashboard_vectors_customer_5'
```
...
- `/mnt/user-data/outputs/` - All Customer 5 CSV files
- `01_postgresql_schema.sql` - Database schema
- `02_load_customer5_data.py` - Data loader
...
**Qdrant Setup:**
- [ ] Collection created: `kpi_dashboard_vectors_customer_5`
- [ ] 354 points uploaded
- [ ] Vector dimension: 3072
- [ ] Test queries work
...
**🎉 Congratulations! Customer 5 test environment is complete and ready for Signal Analyst testing.**
