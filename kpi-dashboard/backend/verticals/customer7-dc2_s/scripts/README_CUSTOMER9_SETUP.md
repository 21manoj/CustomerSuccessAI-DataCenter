# Customer 7 Test Environment - Complete Setup Guide
...
Complete SQL and Python scripts for setting up Customer 7 test environment with Signal Analyst integration, incorporating the latest Qdrant architecture changes from January 4-5, 2026.
...
Complete PostgreSQL schema for Customer 7 test environment.
...
### 2. **02_load_customer7_data.py** (14 KB)
Loads all Customer 7 CSV files into PostgreSQL.
...
- Customer 7 specific validation
...
# Run loader
python3 02_load_customer7_data.py
...
- Collection: `kpi_dashboard_vectors_customer_7`
...
#   🎉 Customer 7 test environment is ready!
```
...
**Step 2: Load Customer 7 Data (3 min)**
```bash
python3 02_load_customer7_data.py
...
## 🎯 Customer 7 Test Data Summary

### Accounts (10 Total)

| Account ID | Type | Journey | Health | ARR | Status |
|------------|------|---------|--------|-----|--------|
| 17000 | Success | Healthy Expansion | 99 | $10.0M | ✅ Reference |
| {ACCOUNT_ID_START+1} | Recovery | Recovery Phase | 87 | $4.2M | ✅ Recovering |
| {ACCOUNT_ID_START+2} | Failure | Churned | 25 | $0 | ❌ Churned |
| 17004 | Growth | Rapid Growth | 98 | $9.6M | ✅ Strategic |
| 17005 | Stable | Steady State | 85 | $4.8M | ✅ Healthy |
| 17006 | At Risk | Declining | 55 | $1.3M | ⚠️ At Risk |
| 17007 | Recovery | Crisis Recovery | 93 | $3.0M | ✅ Recovered |
| 17008 | Strategic | Strategic Expansion | 95 | $8.2M | ✅ Strategic |
| 17009 | At Risk | Budget Constrained | 55 | $850K | ⚠️ At Risk |
| 17010 | Transition | Champion Transition | 72 | $2.4M | ⚠️ Transition |
...
results = qdrant.search(
    collection_name="kpi_dashboard_vectors_customer_7",
    query_vector=embedding,
    limit=20
)
...
account_signals = [
    r for r in results 
    if r.payload.get('account_id') == 17006
]
...
**3. Test with Customer 7:**
```bash
# In UI or API
GET /api/signal_analyst/analyze?account_id=17000&customer_id = 7
...
# Should see: 'kpi_dashboard_vectors_customer_7'
```
...
- `/mnt/user-data/outputs/` - All Customer 7 CSV files
- `01_postgresql_schema.sql` - Database schema
- `02_load_customer7_data.py` - Data loader
...
**Qdrant Setup:**
- [ ] Collection created: `kpi_dashboard_vectors_customer_7`
- [ ] 354 points uploaded
- [ ] Vector dimension: 3072
- [ ] Test queries work
...
**🎉 Congratulations! Customer 7 test environment is complete and ready for Signal Analyst testing.**
