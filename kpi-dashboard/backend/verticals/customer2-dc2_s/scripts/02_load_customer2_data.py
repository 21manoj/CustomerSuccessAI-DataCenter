#!/usr/bin/env python3
"""
Customer 2 Data Loader - Smart Version
Automatically deletes existing Customer 2 data before loading
"""
...
def print_header():
    print("=" * 80)
    print("CUSTOMER 2 DATA LOADER - Smart Version with Auto-Cleanup")
    print("=" * 80)
    print()
...
def check_existing_data(engine):
    """Check if Customer 2 data already exists"""
    print()
    print("=" * 80)
    print("CHECKING FOR EXISTING CUSTOMER 2 DATA")
    print("=" * 80)
    print()
    ...
    try:
        with engine.connect() as conn:
            ...
            if customer_count > 0 or account_count > 0:
                print(f"⚠️  Found existing Customer 2 data:")
                print(f"   Customers: {customer_count}")
                print(f"   Accounts: {account_count}")
                return True
            else:
                print("✅ No existing Customer 2 data found")
                return False
...
def delete_customer2_data(engine):
    """Delete all Customer 2 data in correct order (respecting foreign keys)"""
    print()
    print("🗑️  Deleting existing Customer 2 data...")
    print()
    ...
    delete_statements = [
        ...
        ("kpi_definitions", "DELETE FROM kpi_definitions"),  # Assuming Customer 2 specific
    ...
    try:
        with engine.connect() as conn:
            ...
            try:
                ...
                trans.commit()
                print()
                print("✅ Successfully deleted all Customer 2 data")
                return True
...
def verify_integrity(engine):
    ...
    try:
        with engine.connect() as conn:
            ...
            print()
            print("🎯 Customer 2 Specific Checks:")
            print("─" * 80)
            ...
            print(f"  ✅ Customer 2: {customer_name}")
...
def main():
    ...
    if has_existing:
        if not delete_customer2_data(engine):
