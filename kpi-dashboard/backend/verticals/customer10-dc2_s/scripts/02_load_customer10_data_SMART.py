#!/usr/bin/env python3
"""
Customer 10 Data Loader - Smart Version
Automatically deletes existing Customer 10 data before loading
"""
...
def check_existing_data(engine):
    """Check if Customer 10 data already exists"""
    print()
    print("=" * 80)
    ...
    try:
        with engine.connect() as conn:
            ...
            if customer_count > 0 or account_count > 0:
                print(f"⚠️  Found existing Customer 10 data:")
                print(f"   Customers: {customer_count}")
                print(f"   Accounts: {account_count}")
                return True
            else:
                print("✅ No existing Customer 10 data found")
                return False
...
def delete_customer10_data(engine):
    """Delete all Customer 10 data in correct order (respecting foreign keys)"""
    print()
    print("🗑️  Deleting existing Customer 10 data...")
    print()
    ...
    delete_statements = [
        ...
        ("kpi_definitions", "DELETE FROM kpi_definitions"),  # Assuming Customer 10 specific
    ...
    try:
        with engine.connect() as conn:
            ...
            try:
                ...
                trans.commit()
                print()
                print("✅ Successfully deleted all Customer 10 data")
                return True
...
def verify_integrity(engine):
    ...
    try:
        with engine.connect() as conn:
            ...
            print()
            print("🎯 Customer 10 Specific Checks:")
            print("─" * 80)
            ...
            print(f"  ✅ Customer 10: {customer_name}")
...
def main():
    ...
    if has_existing:
        if not delete_customer10_data(engine):
