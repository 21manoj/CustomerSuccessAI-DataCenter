# Customer 10 Test Environment - Complete Setup Guide
...
Complete SQL and Python scripts for setting up Customer 10 test environment with Signal Analyst integration, incorporating the latest Qdrant architecture changes from January 4-5, 2026.
...
Complete PostgreSQL schema for Customer 10 test environment.
...
### 2. **02_load_customer10_data.py** (14 KB)
Loads all Customer 10 CSV files into PostgreSQL.
...
- Customer 10 specific validation
...
# Run loader
python3 02_load_customer10_data.py
...
- Collection: `kpi_dashboard_vectors_customer_10`
...
#   🎉 Customer 10 test environment is ready!
```
...
**Step 2: Load Customer 10 Data (3 min)**
```bash
python3 02_load_customer10_data.py
...
## 🎯 Customer 10 Test Data Summary

### Accounts (10 Total)

| Account ID | Type | Journey | Health | ARR | Status |
|------------|------|---------|--------|-----|--------|
| 20000 | Success | Healthy Expansion | 99 | $10.0M | ✅ Reference |
| {ACCOUNT_ID_START+1} | Recovery | Recovery Phase | 87 | $4.2M | ✅ Recovering |
| {ACCOUNT_ID_START+2} | Failure | Churned | 25 | $0 | ❌ Churned |
| 20004 | Growth | Rapid Growth | 98 | $9.6M | ✅ Strategic |
| 20005 | Stable | Steady State | 85 | $4.8M | ✅ Healthy |
| 20006 | At Risk | Declining | 55 | $1.3M | ⚠️ At Risk |
| 20007 | Recovery | Crisis Recovery | 93 | $3.0M | ✅ Recovered |
| 20008 | Strategic | Strategic Expansion | 95 | $8.2M | ✅ Strategic |
| 20009 | At Risk | Budget Constrained | 55 | $850K | ⚠️ At Risk |
| 20010 | Transition | Champion Transition | 72 | $2.4M | ⚠️ Transition |
...
results = qdrant.search(
    collection_name="kpi_dashboard_vectors_customer_10",
    query_vector=embedding,
    limit=20
)
...
account_signals = [
    r for r in results 
    if r.payload.get('account_id') == 20006
]
...
**3. Test with Customer 10:**
```bash
# In UI or API
GET /api/signal_analyst/analyze?account_id=20000&customer_id = 10
...
# Should see: 'kpi_dashboard_vectors_customer_10'
```
...
- [x] Create test data (Customer 10)
...
- `/mnt/user-data/outputs/` - All Customer 10 CSV files
- `01_postgresql_schema.sql` - Database schema
- `02_load_customer10_data.py` - Data loader
...
**Qdrant Setup:**
- [ ] Collection created: `kpi_dashboard_vectors_customer_10`
- [ ] 354 points uploaded
- [ ] Vector dimension: 3072
- [ ] Test queries work
...
**🎉 Congratulations! Customer 10 test environment is complete and ready for Signal Analyst testing.**
