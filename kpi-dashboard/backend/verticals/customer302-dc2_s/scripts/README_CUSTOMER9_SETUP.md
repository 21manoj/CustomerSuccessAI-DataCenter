# Customer 302 Test Environment - Complete Setup Guide
...
Complete SQL and Python scripts for setting up Customer 302 test environment with Signal Analyst integration, incorporating the latest Qdrant architecture changes from January 4-5, 2026.
...
Complete PostgreSQL schema for Customer 302 test environment.
...
### 2. **02_load_customer302_data.py** (14 KB)
Loads all Customer 302 CSV files into PostgreSQL.
...
- Customer 302 specific validation
...
# Run loader
python3 02_load_customer302_data.py
...
- Collection: `kpi_dashboard_vectors_customer_302`
...
#   🎉 Customer 302 test environment is ready!
```
...
**Step 2: Load Customer 302 Data (3 min)**
```bash
python3 02_load_customer302_data.py
...
## 🎯 Customer 302 Test Data Summary

### Accounts (10 Total)

| Account ID | Type | Journey | Health | ARR | Status |
|------------|------|---------|--------|-----|--------|
| 312000 | Success | Healthy Expansion | 99 | $10.0M | ✅ Reference |
...
results = qdrant.search(
    collection_name="kpi_dashboard_vectors_customer_302",
    query_vector=embedding,
    limit=20
)
...
**3. Test with Customer 302:**
```bash
# In UI or API
GET /api/signal_analyst/analyze?account_id=312000&customer_id = 302
...
# Should see: 'kpi_dashboard_vectors_customer_302'
```
...
- [x] Create test data (Customer 302)
...
### Data Privacy
Customer 302 is TEST DATA ONLY:
...
- `/mnt/user-data/outputs/` - All Customer 302 CSV files
- `01_postgresql_schema.sql` - Database schema
- `02_load_customer302_data.py` - Data loader
...
**Qdrant Setup:**
- [ ] Collection created: `kpi_dashboard_vectors_customer_302`
- [ ] 354 points uploaded
- [ ] Vector dimension: 3072
- [ ] Test queries work
...
**🎉 Congratulations! Customer 302 test environment is complete and ready for Signal Analyst testing.**
