#!/usr/bin/env python3
"""
Customer 320 Data Loader - Smart Version
Automatically deletes existing Customer 320 data before loading
"""
...
def print_header():
    print("=" * 80)
    print("CUSTOMER 320 DATA LOADER - Smart Version with Auto-Cleanup")
    print("=" * 80)
    print()
...
def check_existing_data(engine):
    """Check if Customer 320 data already exists"""
    print()
    print("=" * 80)
    print("CHECKING FOR EXISTING CUSTOMER 320 DATA")
    print("=" * 80)
    print()
    ...
    try:
        with engine.connect() as conn:
            ...
            if customer_count > 0 or account_count > 0:
                print(f"⚠️  Found existing Customer 320 data:")
                print(f"   Customers: {customer_count}")
                print(f"   Accounts: {account_count}")
                return True
            else:
                print("✅ No existing Customer 320 data found")
                return False
...
def delete_customer320_data(engine):
    """Delete all Customer 320 data in correct order (respecting foreign keys)
    ...
    """
    print()
    print("🗑️  Deleting existing Customer 320 data...")
    print()
    ...
    try:
        ...
        with engine.begin() as conn:  # Use begin() context manager - auto-commits on success, rolls back on error
            ...
            print()
            print("✅ Successfully deleted all Customer 320 data")
            return True
...
def load_table(engine, filename, table_name, description, upload_mode='incremental'):
    ...
    try:
        ...
        if upload_mode == 'full_refresh':
            ...
            print(f"   🗑️  Full Refresh: Deleting existing data for customer 320...")
            with engine.connect() as conn:
...
def verify_integrity(engine):
    ...
    try:
        with engine.connect() as conn:
            ...
            print()
            print("🎯 Customer 320 Specific Checks:")
            print("─" * 80)
            ...
            print(f"  ✅ Customer 320: {customer_name}")
...
def main():
    ...
    if has_existing:
        if not delete_customer320_data(engine):
