#!/usr/bin/env python3
"""
Customer 6 Data Loader - Smart Version
Automatically deletes existing Customer 6 data before loading
"""
...
def print_header():
    print("=" * 80)
    print("CUSTOMER 6 DATA LOADER - Smart Version with Auto-Cleanup")
    print("=" * 80)
    print()
...
def check_existing_data(engine):
    """Check if Customer 6 data already exists"""
    print()
    print("=" * 80)
    print("CHECKING FOR EXISTING CUSTOMER 6 DATA")
    print("=" * 80)
    print()
    ...
    try:
        with engine.connect() as conn:
            ...
            if customer_count > 0 or account_count > 0:
                print(f"⚠️  Found existing Customer 6 data:")
                print(f"   Customers: {customer_count}")
                print(f"   Accounts: {account_count}")
                return True
            else:
                print("✅ No existing Customer 6 data found")
                return False
...
def delete_customer6_data(engine):
    """Delete all Customer 6 data in correct order (respecting foreign keys)"""
    print()
    print("🗑️  Deleting existing Customer 6 data...")
    print()
    ...
    delete_statements = [
        ...
        ("kpi_definitions", "DELETE FROM kpi_definitions"),  # Assuming Customer 6 specific
    ...
    try:
        with engine.connect() as conn:
            ...
            try:
                ...
                trans.commit()
                print()
                print("✅ Successfully deleted all Customer 6 data")
                return True
...
def verify_integrity(engine):
    ...
    try:
        with engine.connect() as conn:
            ...
            print()
            print("🎯 Customer 6 Specific Checks:")
            print("─" * 80)
            ...
            print(f"  ✅ Customer 6: {customer_name}")
...
def main():
    ...
    if has_existing:
        if not delete_customer6_data(engine):
