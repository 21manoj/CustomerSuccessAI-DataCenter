#!/usr/bin/env python3
"""
Customer 13 Data Loader - Smart Version
Automatically deletes existing Customer 13 data before loading
"""
...
def check_existing_data(engine):
    """Check if Customer 13 data already exists"""
    print()
    print("=" * 80)
    ...
    try:
        with engine.connect() as conn:
            ...
            if customer_count > 0 or account_count > 0:
                print(f"⚠️  Found existing Customer 13 data:")
                print(f"   Customers: {customer_count}")
                print(f"   Accounts: {account_count}")
                return True
            else:
                print("✅ No existing Customer 13 data found")
                return False
...
def delete_customer13_data(engine):
    """Delete all Customer 13 data in correct order (respecting foreign keys)"""
    print()
    print("🗑️  Deleting existing Customer 13 data...")
    print()
    ...
    delete_statements = [
        ...
        ("kpi_definitions", "DELETE FROM kpi_definitions"),  # Assuming Customer 13 specific
    ...
    try:
        with engine.connect() as conn:
            ...
            try:
                ...
                trans.commit()
                print()
                print("✅ Successfully deleted all Customer 13 data")
                return True
...
def verify_integrity(engine):
    ...
    try:
        with engine.connect() as conn:
            ...
            print()
            print("🎯 Customer 13 Specific Checks:")
            print("─" * 80)
            ...
            print(f"  ✅ Customer 13: Test Drive Company")
...
def main():
    ...
    if has_existing:
        if not delete_customer13_data(engine):
