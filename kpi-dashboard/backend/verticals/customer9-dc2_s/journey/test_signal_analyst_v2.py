#!/usr/bin/env python3
"""
Multi-Signal Predictor V2 - Production Ready
============================================

Combines KPI health + Event signals for accurate predictions.
Properly calibrated weights and thresholds.
...
# Ensure we can import sqlalchemy
try:
    from sqlalchemy import create_engine, text
except ImportError as e:
    print(f"❌ ERROR: sqlalchemy not installed. Please install it with: pip install sqlalchemy")
    print(f"   Error: {e}")
    sys.exit(1)
...
class EventSignalAnalyzer:
    ...
    def calculate_event_risk_score(self, events):
        ...
        # Calibrated severity weights (realistic impact on health)
        severity_weights = {
            'outage': 4.0,                # Critical infrastructure failure
            'escalation': 3.0,            # Executive escalation
            'executive_engagement': 2.5,  # High-level concern
            'support_ticket': 1.2,        # Technical issues
            'technical_issue': 1.2,       # Technical problems
            'budget_review': 0.8,         # Financial review
            'email': 0.3,                 # Communication
            'meeting': 0.2,               # Regular engagement
            'qbr': 0.4,                   # Quarterly review
            'contract_discussion': 0.2,   # Contract talk
            'champion_interaction': -0.5, # Positive engagement
            'expansion_discussion': -1.0, # Growth signal
            'milestone': -0.8,            # Achievement
        }
        ...
        for event in events:
            ...
            # Sentiment modifier (amplifies risk for negative events)
            if sentiment < 0:
                sentiment_risk = abs(sentiment) * 1.2  # -0.9 → +1.08 risk
            else:
                sentiment_risk = sentiment * -0.8  # +0.9 → -0.72 risk (reduces risk)
            
            risk_score += type_risk + sentiment_risk
        
        # Normalize by event count to avoid penalizing high-engagement accounts
        if len(events) > 6:
    ...
    def detect_escalation_pattern(self, events):
        ...
        # Check if escalations are in recent weeks
...
class MultiSignalPredictor:
    ...
    def get_health_score_at_week(self, account_id, week):
        ...
        with self.engine.connect() as conn:
            ...
            # If no data for this week, try the next week (milestone might be ahead)
            result = conn.execute(text("""
                SELECT health_score
                FROM journey_kpis
                WHERE journey_account_id = :account_id
    ...
    def predict_with_events(self, account_id, week):
        ...
        if event_summary['has_escalation']:
            adjusted_health -= 3.0  # Escalation pattern
        
        if event_summary['has_disengagement']:
            adjusted_health -= 5.0  # Disengagement pattern
        
        if event_summary['sentiment_trend'] < -0.3:
            adjusted_health -= 3.0  # Declining sentiment
    ...
    def predict_outcome_from_health(self, health_score):
        ...
        # Calibrated thresholds based on journey data patterns
        if health_score < 25:
            # Severe crisis, imminent churn
            return {
                'outcome': 'churn',
                'churn_risk': 0.95,
                'expansion_prob': 0.05
...
def test_multi_signal_predictions(db_url):
    ...
    print("="*70)
    print("MULTI-SIGNAL PREDICTOR V2 - PRODUCTION TEST")
    print("="*70)
    print()
    ...
    for milestone in milestones:
        ...
        # Determine actual outcome from milestone
        milestone_lower = milestone_type.lower()
        if 'expansion' in milestone_lower:
        # Expansion signals (even with 'alert')
    ...
    print(f"  V2 (Multi-signal):    {accuracy:.1f}% ({correct_count}/{total_count})")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Test multi-signal predictions V2')
    ...
    if not args.db_url:
        ...
        print("  python3 test_signal_analyst_v2.py")
        sys.exit(1)
