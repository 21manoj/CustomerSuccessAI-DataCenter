...
<outcome_definitions>
CRITICAL: Understand these distinctions carefully:

**expansion** - Customer is thriving and ready to buy more
  - High health score (>82)
  - Positive sentiment and engagement
  - Proactive discussions about growth

**healthy** - Customer is doing well, no concerns
  - Good health score (65-82)
  - Stable or improving metrics
  - Normal engagement levels

**stable** - Customer is okay but not thriving
  - Moderate health score (50-65)
  - Some minor concerns but manageable
  - Relationship is steady

**at_risk** - Customer has problems but hasn't decided to leave
  - Low health score (25-50)
  - Significant issues or concerns
  - May be evaluating alternatives
  - INTERVENTION CAN STILL SAVE THE ACCOUNT

**churn** - Customer has decided to leave or contract is ending
  - Very low health score (<25)
  - Contract termination confirmed
  - Exit interviews or offboarding started
  - Competitor selected and announced
  - ACCOUNT IS LOST - too late for intervention

⚠️  IMPORTANT: Do not confuse "at_risk" with "churn"
- at_risk = Still a customer, having issues, needs immediate help
- churn = Already leaving or contract terminated, relationship ending
</outcome_definitions>
...
class HybridPredictorV4:
    ...
    def _get_account_context(self, account_id):
        """Get account metadata - minimal schema-safe version"""
        with self.engine.connect() as conn:
            # First, let's just get what we know exists for sure
            result = conn.execute(text("""
                SELECT account_name
            ...
            if row:
                return {
                    'account_name': row[0],
                    'industry': 'Technology',      # Defaults for LLM context
                    'segment': 'Enterprise',
                    'arr': 500000,
    ...
    def _reconcile_predictions(self, v3_result, llm_result, account_context):
        """Combine V3 and LLM predictions with smart override logic"""
        ...
        severity_order = {'expansion': 4, 'healthy': 3, 'stable': 2, 'at_risk': 1, 'churn': 0}
        ...
        if v3_prediction == llm_prediction:
            ...
            # Disagreement - apply smart override logic
            
            # Rule 1: LLM must be very confident to override (>0.85)
            llm_confident = llm_result['confidence'] > 0.85
            
            # Rule 2: Check severity distance (don't allow jumps >1 level)
            v3_severity = severity_order.get(v3_prediction, 0)
            llm_severity = severity_order.get(llm_prediction, 0)
            severity_distance = abs(v3_severity - llm_severity)
            reasonable_jump = severity_distance <= 1
            
            # Rule 3: Health score alignment check
            # If health is >40, LLM shouldn't predict churn
            # If health is <25, LLM shouldn't predict healthy/expansion
            health_aligned = True
            if adjusted_health > 40 and llm_prediction == 'churn':
                health_aligned = False  # Health too high for churn
            if adjusted_health < 25 and llm_prediction in ('healthy', 'expansion'):
                health_aligned = False  # Health too low for positive outcomes
            
            # Rule 4: Be conservative with churn predictions
            # Only allow churn if health is <30 OR LLM confidence is >0.90
            if llm_prediction == 'churn':
                churn_justified = (adjusted_health < 30) or (llm_result['confidence'] > 0.90)
            else:
                churn_justified = True
            
            # Decision: Allow override only if ALL checks pass
            allow_override = llm_confident and reasonable_jump and health_aligned and churn_justified
            
            if allow_override:
                # LLM override approved
                confidence = llm_result['confidence']
                final_prediction = llm_prediction
                override = True
            else:
                # Keep V3 prediction (override rejected)
                confidence = 0.75  # Lower confidence when predictions conflict
                final_prediction = v3_prediction
                override = False
                # Add note about why override was rejected
                if not llm_confident:
                    llm_result['override_reason'] = f"LLM confidence too low ({llm_result['confidence']:.2f} < 0.85)"
                elif not reasonable_jump:
                    llm_result['override_reason'] = f"Severity jump too large ({v3_prediction} -> {llm_prediction})"
                elif not health_aligned:
                    llm_result['override_reason'] = f"Health score ({adjusted_health:.1f}) doesn't support {llm_prediction}"
                elif not churn_justified:
                    llm_result['override_reason'] = f"Churn prediction not justified (health: {adjusted_health:.1f})"
        ...
        return {
            ...
            'predictions_agree': v3_prediction == llm_prediction,
            'llm_override': override,
            'override_rejected': not override and v3_prediction != llm_prediction,
...
def test_v4_predictions(db_url, api_key, force_llm=False):
    ...
    for milestone in milestones:
        ...
        if prediction['used_llm']:
            ...
            elif prediction.get('override_rejected'):
                llm_pred = prediction.get('llm_prediction', '?')
                v3_pred = prediction.get('v3_prediction', '?')
                reason = prediction.get('llm_result', {}).get('override_reason', 'Safety check failed')
                print(f"   🛡️  Override Rejected: V3 said '{v3_pred}', LLM wanted '{llm_pred}' but kept V3")
