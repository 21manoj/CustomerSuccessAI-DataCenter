#!/usr/bin/env python3
"""
Customer 321 Data Loader - Smart Version
Automatically deletes existing Customer 321 data before loading
"""
...
def print_header():
    print("=" * 80)
    print("CUSTOMER 321 DATA LOADER - Smart Version with Auto-Cleanup")
    print("=" * 80)
    print()
...
def check_existing_data(engine):
    """Check if Customer 321 data already exists"""
    print()
    print("=" * 80)
    print("CHECKING FOR EXISTING CUSTOMER 321 DATA")
    print("=" * 80)
    print()
    ...
    try:
        with engine.connect() as conn:
            ...
            if customer_count > 0 or account_count > 0:
                print(f"⚠️  Found existing Customer 321 data:")
                print(f"   Customers: {customer_count}")
                print(f"   Accounts: {account_count}")
                return True
            else:
                print("✅ No existing Customer 321 data found")
                return False
...
def delete_customer321_data(engine):
    """Delete all Customer 321 data in correct order (respecting foreign keys)
    ...
    """
    print()
    print("🗑️  Deleting existing Customer 321 data...")
    print()
    ...
    try:
        ...
        with engine.begin() as conn:  # Use begin() context manager - auto-commits on success, rolls back on error
            ...
            print()
            print("✅ Successfully deleted all Customer 321 data")
            return True
...
def load_table(engine, filename, table_name, description, upload_mode='incremental'):
    ...
    try:
        ...
        if upload_mode == 'full_refresh':
            ...
            print(f"   🗑️  Full Refresh: Deleting existing data for customer 321...")
            with engine.connect() as conn:
...
def verify_integrity(engine):
    ...
    try:
        with engine.connect() as conn:
            ...
            print()
            print("🎯 Customer 321 Specific Checks:")
            print("─" * 80)
            ...
            print(f"  ✅ Customer 321: {customer_name}")
...
def main():
    ...
    if has_existing:
        if not delete_customer321_data(engine):
