#!/usr/bin/env python3
"""
Customer 12 Data Loader - Smart Version
Automatically deletes existing Customer 12 data before loading
"""
...
def check_existing_data(engine):
    """Check if Customer 12 data already exists"""
    print()
    print("=" * 80)
    ...
    try:
        with engine.connect() as conn:
            ...
            if customer_count > 0 or account_count > 0:
                print(f"⚠️  Found existing Customer 12 data:")
                print(f"   Customers: {customer_count}")
                print(f"   Accounts: {account_count}")
                return True
            else:
                print("✅ No existing Customer 12 data found")
                return False
...
def delete_customer12_data(engine):
    """Delete all Customer 12 data in correct order (respecting foreign keys)"""
    print()
    print("🗑️  Deleting existing Customer 12 data...")
    print()
    ...
    delete_statements = [
        ...
        ("kpi_definitions", "DELETE FROM kpi_definitions"),  # Assuming Customer 12 specific
    ...
    try:
        with engine.connect() as conn:
            ...
            try:
                ...
                trans.commit()
                print()
                print("✅ Successfully deleted all Customer 12 data")
                return True
...
def verify_integrity(engine):
    ...
    try:
        with engine.connect() as conn:
            ...
            print()
            print("🎯 Customer 12 Specific Checks:")
            print("─" * 80)
            ...
            print(f"  ✅ Customer 12: Rate Limit Test 1768770839 1")
...
def main():
    ...
    if has_existing:
        if not delete_customer12_data(engine):
