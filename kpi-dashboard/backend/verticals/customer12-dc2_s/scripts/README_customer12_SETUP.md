# Customer 12 Test Environment - Complete Setup Guide
...
Complete SQL and Python scripts for setting up Customer 12 test environment with Signal Analyst integration, incorporating the latest Qdrant architecture changes from January 4-5, 2026.

### What's New (Jan 4-5 Architecture Update)

**🔥 CRITICAL QDRANT CHANGES:**
- ✅ **Collection Naming:** `kpi_dashboard_vectors_customer_12` (not separate collections)
...
collection_name = f"kpi_dashboard_vectors_customer_12"
...
Complete PostgreSQL schema for Customer 12 test environment.
...
### 2. **02_load_customer12_data.py** (14 KB)
Loads all Customer 12 CSV files into PostgreSQL.
...
- Customer 12 specific validation
...
# Run loader
python3 02_load_customer12_data.py
...
- Collection: `kpi_dashboard_vectors_customer_12`
...
#   🎉 Customer 12 test environment is ready!
```
...
**Step 2: Load Customer 12 Data (3 min)**
```bash
python3 02_load_customer12_data.py
...
## 🎯 Customer 12 Test Data Summary

### Accounts (10 Total)

| Account ID | Type | Journey | Health | ARR | Status |
|------------|------|---------|--------|-----|--------|
| 22000 | Success | Healthy Expansion | 99 | $10.0M | ✅ Reference |
...
results = qdrant.search(
    collection_name="kpi_dashboard_vectors_customer_12",
    query_vector=embedding,
    limit=20
)
...
**3. Test with Customer 12:**
```bash
# In UI or API
GET /api/signal_analyst/analyze?account_id=22000&CUSTOMER_ID = 12
...
# Should see: 'kpi_dashboard_vectors_customer_12'
```
...
- [x] Create test data (Customer 12)
...
- `/mnt/user-data/outputs/` - All Customer 12 CSV files
- `01_postgresql_schema.sql` - Database schema
- `02_load_customer12_data.py` - Data loader
...
**Qdrant Setup:**
- [ ] Collection created: `kpi_dashboard_vectors_customer_12`
- [ ] 354 points uploaded
- [ ] Vector dimension: 3072
- [ ] Test queries work
...
**🎉 Congratulations! Customer 12 test environment is complete and ready for Signal Analyst testing.**
