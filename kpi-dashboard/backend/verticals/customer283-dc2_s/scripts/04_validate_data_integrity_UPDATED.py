#!/usr/bin/env python3
"""
Validate Customer 283 Data & Qdrant Integration
==============================================
...
Updated: Jan 6, 2026 (Customer 283 Production)
...
print("=" * 80)
print("CUSTOMER 283 DATA & QDRANT VALIDATION")
print("=" * 80)
...
with engine.connect() as conn:
    ...
    for table, exp in expected.items():
        try:
            if table in ["accounts", "qualitative_signals", "kpi_measurements", "account_profiles",
                        ...
                        "account_products"]:
                # Check Customer 283 specific data
                if table == "accounts":
                    query = f"SELECT COUNT(*) FROM {table} WHERE customer_id = 283"
                elif table in ["kpi_definitions", "products", "partner_definitions"]:
                    query = f"SELECT COUNT(*) FROM {table}"
                else:
                    query = f"SELECT COUNT(*) FROM {table} WHERE account_id IN (SELECT account_id FROM accounts WHERE customer_id = 283)"
            else:
                query = f"SELECT COUNT(*) FROM {table}"
    ...
    quality_checks = [
        {
            ...
            "query": """
                ...
                WHERE ap.account_id IS NULL AND a.customer_id = 283
            """,
            "expected": 0
        },
        {
    ...
    # Customer 283 specific checks
    print(f"\n🎯 Customer 283 Specific Checks:")
    print("─" * 80)
    
    # Account count
    result = conn.execute(text("""
        SELECT COUNT(*) FROM accounts WHERE customer_id = 283
    """))
    ...
    result = conn.execute(text("""
        ...
        FROM (
            ...
            WHERE account_id IN (SELECT account_id FROM accounts WHERE customer_id = 283)
            GROUP BY account_id
        ) sub
        FULL OUTER JOIN account_products ap ON 1=1
        WHERE ap.account_id IN (SELECT account_id FROM accounts WHERE customer_id = 283)
    """))
...
test_accounts = [293000, {ACCOUNT_ID_START+2}, 10007]  # Success, churned, recovery
...
if all_ready and len(table_issues) == 0 and len(quality_issues) == 0:
    ...
    print("\n📋 Next Steps:")
    print("  1. Test Signal Analyst with Customer 283 accounts")
    ...
    print("\n📋 Recommended Actions:")
    if table_issues:
        print("  1. Re-run data loader: python3 scripts/02_load_customer283_data.py")
    if not all_ready:
