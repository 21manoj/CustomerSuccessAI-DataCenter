#!/usr/bin/env python3
"""
Phase 1b: Backfill UUIDs for Data Center Customer (customer_id=9) ONLY

This script:
  1. Sets vertical='dc' and generates uuid for customer_id=9
  2. Generates UUIDs for ALL child records (accounts, products, users, KPIs, etc.)
  3. Propagates parent UUIDs to child FK columns (customer_uuid, account_uuid, etc.)

Safety:
  - Only touches records where customer_id=9
  - Skips records that already have a uuid (idempotent)
  - Wraps everything in a single transaction (all-or-nothing)
  - Prints detailed progress for auditability

Rollback: phase1d_rollback.py clears all uuid/vertical columns.
"""
...
def run(db):
    """
    Backfill UUIDs for customer_id=9 and all its children.
    ...
    with db.engine.connect() as conn:
        ...
        try:
            ...
            account_uuid_map = {}  # account_id -> uuid (for child lookups)
            count = 0
            ...
            # Also set customer_uuid for accounts that already had uuid but missing customer_uuid
            conn.execute(text(
            ...
            for kpi in kpis:
                ...
                # product_uuid: look up from products table if product_id is set
                prod_uuid = None
                if prod_id:
            ...
            for row in ts_rows:
                ...
                acct_uuid = account_uuid_map.get(acct_id)
                # Look up kpi_uuid from kpis table
            ...
            # ============================================================
            # Step 10: KPI Reference Ranges
            # ============================================================
            print(f"\n[Step 10] Backfill kpi_reference_ranges")
            count = _backfill_simple_child(
                conn, 'kpi_reference_ranges', 'range_id',
                TARGET_CUSTOMER_ID, customer_uuid, TARGET_VERTICAL, 'kpi_reference_range',
                generate_id
            )
            stats['kpi_reference_ranges'] = count

            # ============================================================
            # Step 11-18: Remaining tables (simpler structure)
            # ============================================================
            simple_tables = [
            ...
            # Tables with account_id + customer_id
            account_tables = [
            ...
            trans.commit()
            print("\n" + "=" * 60)
            print("COMMITTED - All backfills applied successfully")
            print("=" * 60)

        except Exception as e:
            trans.rollback()
            print(f"\nROLLBACK - Error during backfill: {e}")
            import traceback
            traceback.print_exc()
...
def _backfill_simple_child(conn, table, pk_col, customer_id, customer_uuid,
                            vertical, entity_type, generate_id_fn):
    """Backfill a table that only has customer_id FK (no account_id)."""
    rows = conn.execute(text(
        f"SELECT {pk_col}, uuid FROM {table} WHERE customer_id = :cid"
    ), {'cid': customer_id}).fetchall()

    count = 0
    for row in rows:
        row_id, existing_uuid = row[0], row[1]
        if existing_uuid:
            continue
        new_uuid = generate_id_fn(vertical, entity_type)
        conn.execute(text(
            f"UPDATE {table} SET uuid = :uuid, customer_uuid = :cuuid WHERE {pk_col} = :rid"
        ), {'uuid': new_uuid, 'cuuid': customer_uuid, 'rid': row_id})
        count += 1
    print(f"  Backfilled {count} rows ({len(rows)} total)")
    return count


def _backfill_account_child(conn, table, pk_col, customer_id, customer_uuid,
                             account_uuid_map, vertical, entity_type, generate_id_fn):
    """Backfill a table that has both customer_id and account_id FKs."""
    rows = conn.execute(text(
        f"SELECT {pk_col}, account_id, uuid FROM {table} WHERE customer_id = :cid"
    ), {'cid': customer_id}).fetchall()

    count = 0
    for row in rows:
        row_id, acct_id, existing_uuid = row[0], row[1], row[2]
        if existing_uuid:
            continue
        new_uuid = generate_id_fn(vertical, entity_type)
        acct_uuid = account_uuid_map.get(acct_id)
        conn.execute(text(
            f"UPDATE {table} SET uuid = :uuid, customer_uuid = :cuuid, account_uuid = :auuid "
            f"WHERE {pk_col} = :rid"
        ), {'uuid': new_uuid, 'cuuid': customer_uuid, 'auuid': acct_uuid, 'rid': row_id})
        count += 1
    print(f"  Backfilled {count} rows ({len(rows)} total)")
    return count
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print(f"Phase 1b: Backfill UUIDs for customer_id={TARGET_CUSTOMER_ID} (DC)")
    print("=" * 60)
